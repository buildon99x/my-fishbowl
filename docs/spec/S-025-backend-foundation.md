...
- **페르소나 합치**: 주 사용자 4–6세(글 못 읽음) 어린이, 보조 사용자 부모(설정/백업/복원 담당)라는 페르소나(`docs/spec/S-023-onboarding.md`, `S-021-magic-moment.md`, `S-024-touch-target-upgrade.md` 근거)를 모든 결정 항목에 반영한다. 백업/충돌/오류 관련 UI는 기본적으로 **부모 영역**에 둔다.
...
> 어린이/부모 두 갈래로 분리해 기술한다. 어린이는 항상 **방해 없는** 어항 사용을 보장받고, 백업/충돌/계정은 모두 **부모 영역**에서 처리된다.

### 어린이 흐름 (기본)

1. 어린이가 처음 앱을 연다 → 클라이언트가 디바이스 ID를 무음으로 발급/저장한다. 어떤 가입 화면도, 어떤 sync 위젯도 노출하지 않는다.
2. 어린이가 어항을 만들고 물고기를 등록한다 → `localStorage`에 즉시 반영 + 서버 push는 **온보딩/마법 모먼트가 끝난 뒤**에만 백그라운드로 시도된다.
3. 네트워크 실패/충돌/페이로드 초과 등 모든 오류 상황에서 어린이 영역에는 어떤 modal/blocking 다이얼로그도 띄우지 않는다. 어린이는 항상 자기 어항을 계속 만질 수 있다.
4. 다른 기기에서 복원할 때 sprite 로딩이 지연되면 빈 어항 대신 회색 실루엣 + 거품 placeholder(S-011 재사용)로 “준비 중”을 표현한다.

### 부모 흐름 (설정 영역 게이트)

1. 부모는 부모 영역에 진입한 뒤 “Google/Apple로 백업”·“복구 코드”·“계정 삭제”·sync 상태 확인·충돌 해결 등에 접근한다. 부모 영역의 진입 게이트는 본 스펙에서 결정하지 않고 별도 스펙으로 분리한다(Open Questions 참조).
2. 처음 백업을 켜는 시점에 부모 영역에 1회 안내 배지(“이 태블릿의 어항을 우리 서버로 보냅니다”)를 띄운다. OAuth는 별도 명시 동의.
3. 다른 기기에서 같은 OAuth 계정으로 로그인했을 때만 부모 영역에 충돌 카드가 표시된다. **머지/덮어쓰기/포기** 결정은 부모만 내릴 수 있다.
4. 부모가 백업을 끄거나 계정을 삭제하면 서버 데이터가 즉시 삭제되고, 클라이언트는 다시 `localStorage`-only 모드로 동작한다.

## UI/상태 요구사항(설계 수준)

- 어린이 영역:
  - 어떤 백업 버튼/sync indicator/충돌 다이얼로그/오류 모달도 노출되지 않는다.
  - sprite GET 실패/지연 시 회색 실루엣 + 거품 placeholder로 자연스러운 대체.
  - 부모 영역 진입 버튼(예: ❓ 도움말과 동일 위치 또는 인접)만 자식 영역에서 보인다. 진입 게이트는 별도 스펙에서 결정.
- 부모 영역:
  - "Google/Apple로 백업" 버튼. **OAuth 시크릿이 비어 있으면 버튼 자체가 노출되지 않는다.**
  - sync indicator(상세 4상태). 색은 단조 변화만, 깜빡임 없음.
  - 충돌 해결 카드(merge / overwrite / abandon). `merge`는 OAuth 연결 후에만 렌더링, 그 외에는 아예 표시하지 않는다(비활성 버튼 제거).
  - 90KB 초과/마이그레이션 안내/계정 삭제 confirmation 배지.
- 필요한 상태:
  - 클라이언트 메모리: `deviceId`, `sessionToken?`, `accountLink?`, `syncStatus`(disabled/idle/syncing/error/conflict), `lastServerUpdatedAt`, `pendingConflict?`.
  - localStorage: `my-fishbowl:aquarium`, `my-fishbowl:deviceId`, `my-fishbowl:backupOptIn`(부모 동의 캐시), `my-fishbowl:parentNoticeSeen`(첫 PUT 안내 노출 여부).
- 오류 또는 빈 상태:
  - 서버 5xx/네트워크 실패: **어린이 영역 무변화**. 부모 영역 sync indicator만 단조 색 변화. 자동 backoff.
  - 동기화 충돌(412): 자동 안전 보존 모드로 진입(서버/로컬 둘 다 보존, 클라이언트는 로컬 본으로 계속 동작). 부모 영역에 충돌 카드 표시. 어린이 영역은 무변화.
  - 디바이스 한도 초과: 어린이 영역은 무변화. 부모 영역에서 “저장 한도” 안내.
- 모션/접근성:
  - 어린이 영역 노출 가능성이 있는 모든 시각 신호(예: 폴백 거품)는 `prefers-reduced-motion` 가드를 따른다(`S-021`, `S-023`과 동일 정책).
  - 부모 영역 상태 표시는 단조 색 변화만 사용, 점멸 금지.

## 어린이/부모 영역 분리 정책

본 스펙의 **모든 결정 항목**은 다음 매핑 표 위에서 해석한다. 하위 스펙(S-025a~e)은 이 표를 상속하고, 새 결정이 추가되면 본 표에 한 줄을 추가한다.

| 결정 항목 / 기능 | 어린이 영역 | 부모 영역 | 비고 |
| --- | --- | --- | --- |
| 디바이스 ID 발급/유지 | 무음(자동) | 표시 없음 | 어떤 안내도 띄우지 않음 |
| 첫 PUT(마이그레이션) | 표시 없음 | 1회 안내 배지 | `parentNoticeSeen`로 1회 게이트 |
| sync indicator(상세 4상태) | **노출 안 함** | 노출 | 단조 색, 깜빡임 금지 |
| sync 폴백(거품 placeholder, 회색 실루엣) | 노출 | 표시 없음 | 어린이용 정서 보존 |
| 충돌(412) 카드 + 결정 버튼 | 노출 안 함, 자동 안전 보존 | 노출 + 결정 | merge는 OAuth 연결 후만 렌더 |
| OAuth 시작/콜백/언링크 | 노출 안 함 | 부모 게이트 뒤 | 시크릿 부재 시 부모 영역에도 노출 안 함 |
| 복구 코드 발급(S-025c) | 노출 안 함 | 부모 게이트 뒤 | 어린이 우발 발급 금지 |
| 복구 코드 입력(S-025c) | 노출 안 함 | 부모 게이트 뒤 | 부모 호출 흐름 |
| 계정 삭제 confirmation | 노출 안 함 | 부모 게이트 뒤 + 추가 confirm | 파괴적 동작 보호 |
| 90KB 초과/페이로드 거절 안내 | 노출 안 함 | sync indicator + 안내 | 어린이에게 텍스트 경고 금지 |
| 어항 한도(1개) 초과 안내 | 노출 안 함 | 부모 영역 안내 | 형제 시나리오는 별도(Open Questions) |
| God Mode 토글(dev) | dev에서만 | dev에서만 | production에는 없음 |
| 부모 영역 진입 버튼 | 노출 | — | 진입 게이트는 별도 스펙 |

**원칙**:
1. 어린이 영역은 **읽지 못해도, 결정하지 못해도, 끊기지 않는** 어항을 보장한다.
2. 어른 멘탈 모델이 필요한 모든 UI(timestamp 비교, etag, 백업 ON/OFF, 계정 관리)는 부모 영역에만 둔다.
3. 어린이 영역에 노출되는 어떤 표시도 텍스트 의존이 아니어야 하며, 깜빡임/3Hz 이상 변화가 없어야 한다.
4. 새 결정이 추가되면 위 표에 행을 추가하고, 해당 결정 항목 본문에 “영역: 어린이/부모/없음”을 명시한다.
...
- [ ] 익명 → OAuth 연결 충돌 시 merge/overwrite/abandon UX 분기가 **부모 영역**에 한정해 명시되어 있다.
- [ ] **어린이/부모 영역 분리 매핑 표**가 본 스펙에 존재하고, 모든 결정 항목이 매핑 표에 반영되어 있다.
- [ ] 어린이 영역에는 텍스트/blocking 모달/sync indicator/충돌 결정 UI가 **절대** 노출되지 않는 정책이 명시되어 있다.
- [ ] 자동 sync가 **온보딩(S-023) 종료 + 마법 모먼트(S-021) 큐 비어있음** 조건을 통과한 뒤에만 트리거된다는 정책이 명시되어 있다.
- [ ] sprite GET 실패/지연 시 어린이 영역에 회색 실루엣 + 거품 placeholder fallback이 명시되어 있다.
- [ ] 디바이스 분실/부모 부재 시 마지막 7일 스냅샷 보존 hook이 위협 모델에 명시되어 있다.
...
- **부모 영역 진입 게이트**(긴 탭 / 4자리 숫자 / 두 손가락 탭 등) 결정 위치. → 본 스펙에서 결정하지 않는다. S-023 후속 또는 신규 스펙(예: `S-026-parent-area`)으로 분리. S-025/S-025a는 “부모 영역이 존재한다는 전제”만 의존한다.
- 가정 내 형제(같은 태블릿/브라우저에서 둘 이상의 어린이) 시나리오. → 본 스펙의 “디바이스 단위 어항 1개” 정책과 충돌한다. 본 스펙 범위는 변경하지 않고, “디바이스 내 어항 프로필 분리”를 별도 미래 스펙(예: `S-025f`)으로 둔다.
- 디바이스 분실 시 마지막 7일 스냅샷 자동 보존 위치/형식. → 위협 모델에는 hook을 두었으나 구체 키/만료/복원 흐름은 S-025e 확장 또는 별도 스펙에서 결정.
